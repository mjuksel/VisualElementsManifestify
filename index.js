...
const Arg = process.argv.filter((v,i) => i >= 2 && v)
...
fs.readdir('.', 'utf8', (error, files) => {
	...
	for (const file of files) {
		if (file.startsWith('icon')) {
			...
			let backgroundColor = Arg[0] || '#123123'
			let textLightDark = Arg[1] || 'dark'
			let showNameOnOff = Arg[2] || 'off'
			fs.writeFileSync(`${applicationName}.VisualElementsManifest.xml`,`
